...
${ssh_cmd} ${guest_addr} sudo mount /dev/vda1 /mnt || exit 1
${ssh_cmd} ${guest_addr} sudo chmod 777 /mnt || exit 1

for op in randread randwrite read write; do
	echo Running test $op...
	${ssh_cmd} ${guest_addr} fio ${guest_tmpdir}/test-$op.fio --fallocate=none --directory=/mnt --output-format=json+ --blocksize=65536 --output=${guest_tmpdir}/results-${op}.json || exit 1
done
